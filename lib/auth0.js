...
const auth0Config = {
  ...
  async onCallback(error, context, session) {
    ...
    if (session?.user) {
      try {
        const accessToken = session.tokenSet?.accessToken || session.accessToken || session.access_token;
        
        if (accessToken) {
          const { upsertUserToSupabaseWithJWT } = await import('./auth0-upsert');
          await upsertUserToSupabaseWithJWT(accessToken);
        } else {
          // eslint-disable-next-line no-console
          console.warn('[auth0] No access token found in session');
        }
      } catch (e) {
        // eslint-disable-next-line no-console
