...
const auth0Config = {
  ...
  async onCallback(error, context, session) {
    ...
    if (error) {
      logger.error('[auth0] onCallback error:', error);
      return NextResponse.redirect(
        new URL(`/error?error=${error.message}`, process.env.APP_BASE_URL)
      );
    }
    
    // ユーザー情報をSupabaseにupsert（auth0-upsert.tsの関数を使用）
    if (session?.user?.sub && session?.user?.email) {
      try {
        // 動的インポートでauth0-upsert.tsの関数を呼び出す
        ...
        logger.error('[auth0] Failed to upsert user to Supabase:', e);
        // エラーでもセッション継続のためthrowしない
      }
    }
...
try {
  ...
  logger.error('[auth0] failed to initialize Auth0Client:', errMsg);
  ...
  _auth0 = {
    ...
    async middleware(_req) {
      logger.error('[auth0] middleware stub invoked — Auth0 is not initialized');
